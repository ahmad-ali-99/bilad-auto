...
export default function QuoteBuilder({ prefill, onDraftChange, onPrefillUsed, onUnsavedChange }) {
  ...
  // الزيادة والخصم: مفتوحان للمشرفين ولبكر
  const [mayPriceAdjust, setMayPriceAdjust] = useState(false);
  ...
  useEffect(() => {
    // صلاحية الزيادة والخصم تنفحص بالاسم (مشرف أو بكر) — منفصلة عن صلاحية الإشراف
    getCurrentUsername().then((n) => setMayPriceAdjust(canPriceAdjust(n))).catch(() => {});
    getIsAdmin()
      .then((admin) => {
        setIsAdmin(admin);
        if (!admin) return;
  ...
  }, [debouncedInputs, validInputs, mayPriceAdjust]);
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      if (result.printed) {
        ...
        setSaveMessage(
          result.attachmentSeparate
            ? 'انفتحت شاشة الطباعة — اختر «مشاركة» أو «حفظ إلى الملفات». وملف التصميم المرفق نزل لحاله بملف منفصل ✔'
            : 'انفتحت شاشة الطباعة — اختر «مشاركة» لإرساله بالواتساب، أو «حفظ إلى الملفات» لتنزيله ✔'
        );
      ...
      <div className="card">
        ...
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px dashed #d5dde6' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            >
              💹 {mayPriceAdjust ? 'زيادة / خصم / تقسيط' : 'التقسيط'} {pricingOpen ? '▲' : '▼'}
            </button>
            {!pricingOpen && (mayPriceAdjust && (Number(markupPercent) > 0 || Number(discountPercent) > 0) || installment) && (
              <span style={{ display: 'inline-flex', gap: 6, flexWrap: 'wrap' }}>
                {mayPriceAdjust && Number(markupPercent) > 0 && (
                ...
                {mayPriceAdjust && Number(discountPercent) > 0 && (
          ...
          {pricingOpen && (
          <>
          {/* الزيادة والخصم: للمشرفين ولبكر — البياع الاعتيادي يشوف التقسيط فقط */}
          {mayPriceAdjust && (
